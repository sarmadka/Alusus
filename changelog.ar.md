...
## الإصدار 0.16.0 (2026-10-××م \ 1448-××-××هـ)

* إعادة هيكلة شفرة القلب (Core) لتنظيم أصناف البيانات:
  - نقل `الـقلب.بـيانات.شـبم` (`Core::Data::Ast`) إلى `الـقلب.شـبم` (`Core::Ast`).
  - نقل `Core::Data::Grammar` إلى `Core::Grammar`.
  - إزالة `الـقلب.بـيانات` (`Core::Data`).
  - اشتقاق جميع أصناف ش.ب.م من الصنف `الـقلب.شـبم.عـقدة` (`Core::Ast::Node`).
  - اشتقاق جميع أصناف بيانات القواعد من الصنف `Core::Grammar::Node`.
* تطبيق مستوى O3 من تحسينات الأداء على عمليات البناء المسبقة التي تتم باستخدام حزمة `بـناء` (`Build`).


## الإصدار 0.15.3 (2026-08-28م \ 1448-03-15هـ)

* إضافة ماكروهات جديدة لمكتبة التنفيذ المعيارية:
  - `احجز_كائنا` (`allocObj`)
  - `أنشئ_كائنا` (`newObj`)
  - `حرر_كائنا` (`freeObj`)
  - `حرر_مؤشر_كائن` (`freeObjPtr`)
* إصلاح علة في حزمة `بـناء` (`Build`) تتسبب في فيض الصوان (buffer overflow).
* إصلاح علة في دالة `مـصفوفة.حدد_الطول` (`Array.setLength`).
* إصلاح علة في دالة `مـدير_شبم.هات_المعطيات_النصية_لمبدل` (`AstMgr.getModifierStringParams`).
* استبدال نسخة ماركداون من الوثائق بنسخة الHTML.
* تقسيم وثائق دليل المكتبات التنفيذية إلى ملفات متعددة.


## الإصدار 0.15.2 (2026-07-10م \ 1448-01-25هـ)

* إصلاح مشكلة في الدالة `مـتم.تـطبيق.حدد_عند` (`Srl.Map.setAt`).


## الإصدار 0.15.1 (2026-06-14م \ 1447-12-28هـ)

* إصلاح مشكلة دوران لا نهائي في المترجم عند مواجهة حلقة حقن (`@حقنة` \ `@injection`) مغلقة.
* إصلاح علة في الصنف `نـمط.مـطابق` (`Regex.Matcher`) عند التعامل مع الأقواس لتحديد مجموعات فرعية.


## الإصدار 0.15.0 (2026-05-16م \ 1447-11-29هـ)

### تحديثات المترجم

* تحديث منشئ الشفرة التنفيذية إلى الإصدار 18 من LLVM.
* تمكين تمرير الخيارات المخصصة عبر سطر الأوامر لتُستخدم لاحقا من قبل المترجم أو المكتبات.

### تحديثات المكتبات المعيارية

* تحديثات مدير الحزم:
  - تغيير اسم `مـحا.اشمل_ملف` (`Apm.importFile`) إلى `مـحا.اشمل.حزمة` (`Apm.importPackage`).
  - جعل رقم الإصدار إلزاميا عند تحديد الحزمة المراد شمولها.
  - تمكين تحديد رقم إصدار جزئي للحزم وإتاحة المجال لمدير الحزم بأن يجد الحزمة الأحدث التي تطابق ذلك الرقم.
  - إضافة خيار لطلب تحديث الحزم.
  - إضافة خيار لطلب إعادة تحميل الحزم.
  - إضافة أمر سطر الأوامر `apm info` لجلب أرقام الإصدارات المتوفرة لحزمة محددة.
* إضافة وظيفة `مـصفوفة.حدد_الطول` (`Array.setLength`).
* إصلاح بعض المشاكل في صنف `مـصفوفة` (`Array`).
* إصلاح تعريف دالة `نـظام.نم` (`System.sleep`).


## الإصدار 0.14.2 (2025-01-11م \ 1447-07-22هـ)
<detail>

* إصلاح مشكلة توافق مع واجهة التطبيق الثنائية (ABI) الخاصة بلغة السي عند تمرير أو استلام بيانات
  من صنف مستخدم (struct).
</detail>


## الإصدار 0.14.1 (2025-11-15م \ 1447-05-24هـ)
<detail>

* إصلاح علة في دالة `نـبم.مدير_شبم.جد_مبدلا_لعنصر` (`Spp.astMgr.findModifierForElement`).
* إصلاح علة في صنف `مـنشئ_نص` (`StringBuilder`).
</detail>


## الإصدار 0.14.0 (2025-11-11م \ 1447-05-20هـ)
<detail>

* تعديل اسلوب بناء الاعتماديات لتلافي الحلقات المغلقة تلقائيا عندما لا تكون ناجمة عن خلل في شفرة
  المستخدم نفسها.
* إضافة الدالة `نـم.أهو_مجلد`.
* إضافة الماكرو `حاذ_هذا` للمساعدة في التوريث المتعدد.
* إصلاح اسم الدالة `وقـت.هات_وقت_مفصل`.
</detail>


## الإصدار 0.13.1 (2025-02-08م \ 1446-08-09هـ)
<detail>

* إصلاح علة في المترجم عن استخدام الصنف `مـمر` (`Passage`) مع معطيات القوالب.
* إصلاح علة تتسبب في شلل المترجم عند انتهاء التنفيذ في حالة تسبب الصنف `مـمر` (`Passage`) بحلقة مغلقة.
</detail>


## الإصدار 0.13.0 (2024-11-12م \ 1446-05-09هـ)
<detail>

### الإضافات والتعديلات على المترجم

* إضافة إمكانية استخدام مؤشر إلى المنادى عند إنشاء الشفرات أثناء العبارات التمهيدية دون الحاجة للإشارة
  لذلك المنادى باستخدام اسمه الذي يرد في الشفرة المصدرية.
* تغيير عملية التمثيل (casting) بين المؤشرات والأعداد الصحيحة لتكون حصرا مع الصنف `صـحيح_متكيف` (`ArchInt`)
  دون الصنف `صـحيح` (`Int`) حتى لو كان عدد بتات الأخير مساوي لعدد بتات المؤشر. هذا التغيير يجعل التعامل مع
  شجرة البناء المجردة غير معتمد على المعمارية.
* جعل التمثيل بين الصنف `صـحيح_متكيف` (`ArchInt`) والأصناف الصحيحة الأخرى يعتبر عدد بتات `صـحيح_متكيف` غير
  محدد (أي قد يكون 32 أو 64) وبناء على ذلك يمنع التمثيل التلقائي إن وجدت احتمالية فقدان في القيمة. سابقًا
  كان يحسب عدد بتات `صـحيح_متكيف` حسب المعمارية الحالية ويبيح التمثيل بناء على ذلك، ما يؤدي لجعل شجرة
  البناء المجردة معتمدة على المعمارية.

### الإضافات والتعديلات على المكتبات المعيارية

* إضافة الدالات التالية إلى `نـبم.مدير_شبم` (`Spp.astMgr`):
...
* إصلاح مشكلة كون الصنف `تـعريف` (`Definition`) لا يضبط ملكية المبدلات التابعة له بشكل صحيح.

### إصلاحات المكتبات المعيارية

* إصلاح مشكلة في دالة `أنشئ` في أصناف ش.ب.م الأساسية.
* إصلاح علات في دوال `نـبم.مدير_بشم` (`Spp.astMgr`) التالية:
...
## الإصدار 0.12.1 (2024-07-05م \ 1445-12-29هـ)
<details>
...
## الإصدار 0.12.0 (2024-07-05م \ 1445-12-29هـ)
<details>

### الإضافات والتعديلات على المترجم

* إضافة مبدل `@أولوية` (`@priority`) لتمكين المستخدم من إعطاء أولوية التهيئة لمتغير عمومي.

### الإضافات والتعديلات على المكتبات المعيارية

* إضافة الدعم لخاصية الخلط (Mixin) في البرمجة كائنية المنحى عن طريق الدالة
  `مدير_شبم.احشر_مكون` (`astMgr.insertMixin`).
* إضافة الدالات التالية إلى `نـبم.مدير_شبم` (`Spp.astMgr`):
  * `هات_اسم_تعريف` (`getDefinitionName`)
  * `هات_أصناف_معطيات_دالة` (`getFuncArgTypes`)
  * `هات_صنف_معطى_دالة` (`getFuncArgType`)
* تمكين اختيار اسم المجمع (linker) ورمز target triple في وحدة `بـناء` (`Build`).
* تمكين تخصيص عمليات حجز الذاكرة في وحدة `ذاكـرة` (`Memory`).
* إضافة الصنف `مـنشئ_نص` (`StringBuilder`) لتمكين إنشاء النصوص بأداء عالي.

### إصلاحات المترجم

* إصلاح مشكلة في المترجم تتسبب في ترجمة التعريفات العمومية داخل الوحدات حتى إن لم تكن تلك التعريفات
  مستخدمة من قبل الشفرة قيد الترجمة.
* إصلاح مشكلة في تسلسل تنفيذ دالات التهيئة العمومية (global constructors).

### إصلاحات المكتبات المعيارية

* إصلاح مشكلة في `لـا_مضمون` (`Possible`) لمنع التمثيل غير المقصود كقيمة دون المرور بعملية التمثيل
  المخصصة.
* إصلاح بعض المشاكل في صنف `نـص` (`String`).
</details>


## الإصدار 0.11.3 (2023-09-11م \ 1445-02-26هـ)
<details>

### الإضافات والتعديلات على المترجم

* تمكين التمثيل التلقائي عبر عدة دالات تمثيل مخصص. تمكن هذه الخاصية التمثيل التلقائي في الحالات التي
  لا يمكن فيها بلوغ الصنف المستهدف إلا عبر عدة دالات تمثيل مخصص. في السابق كان التمثيل التلقائي يعمل
  مع مرحلة واحدة من التمثيل المخصص.

### إصلاحات المترجم

* إصلاح مشكلة في تحسس الخطأ عند تمرير دالة كقيمة ثنائية في جملة شرطية.
* إصلاح مشكلة في إعطاء الأولوية للمنادى المحلي على المنادى المحقون. هذا الإصلاح يحل مشكلة في تعريف
  الدالات المهيمنة (function overriding) في الأصناف الفرعية.

### إصلاحات المكتبات المعيارية

* إصلاح عمليات التعيين والتمثيل للصنف `بـعدم` (`Nullable`).
</details>


## الإصدار 0.11.2 (2023-08-22م \ 1445-02-06هـ)
<details>

### إصلاحات المترجم

* إصلاح تمييز الجمل الإنتهائية داخل داخل رزم الأوامر (مؤثر `~استخدم_في`).
</details>


## الإصدار 0.11.1 (2023-08-15م \ 1445-01-28هـ)
<details>

### إصلاحات المترجم

* إصلاح مشكلة في التعامل مع الترجمة المتداخلة (الترجمة التي تُطلق أثناء معالجة تمهيد ترجمة أخرى).
</details>


## الإصدار 0.11.0 (2023-07-26م \ 1445-01-08هـ)
<details>

### إصلاحات المترجم

* إصلاح مشكلة في المؤثر << (الإزاحة البتية لليمين) تتسبب في تحويل المعطى الطبيعي الصنف إلى حقيقي الصنف.

### الإضافات والتعديلات على المكتبات المعيارية

* إضافة `نـص.اقرأ_محرف_هكس` (`String.parseHexDigit`).
* إضافة `شـبكة.شفر_عنوانيا` (`Net.uriEncode`).
* إضافة `شـبكة.فك_عنوانيا` (`Net.uriDecode`).
* تغيير صنف `خـطأ.هات_الرمز()` (`Error.getCode`) من صحيح إلى نـص.
</details>


## الإصدار 0.10.0 (2023-07-08م \ 1444-12-20هـ)
<details>
...
* إضافة دالة `أهو_عدم` (isNull) ل`سـندنا` (SrdRef) و `سـندهم` (WkRef) و `سـندي` (UnqRef).
* إضافة دالة `أهو_عدم` (isNull) للمغلفات.
...
* إضافة دالتي `هيئ` (`initialize`) و `حرر` (`release`) إلى الصنف `مـتم.نـمط.مـطابق` (`Srl.Regex.Matcher`).
* إضافة دالة `اجتزئ` (`slice`) إلى الصنف `مـتم.مـصفوفة` (`Srl.Array`).
* تمكين إضافة عدة اعتماديات بصيغة مصفوفة نصوص في استدعاء واحد في الوحدة `بـناء` (`Build`).
* تحديث وحدة `بـناء` (`Build`) لتمكين الربط (linking) مع مكتبات خاصة (ليست من مكتبات النظام). شفرة البناء ستقوم
  تلقائيًا بنسخ المكتبات الخاصة إلى مجلد البرنامج المُنتج.
* إضافة ماكرو `هات_مجلد_هذه_الشفرة` (`getThisSourceDirectory`) لجلب اسم المجلد الحاوي على ملف الشفرة
  المصدرية الحالي.
* إضافة صيغة من `مـصفوفة.أضف` (`Array.add`) لإضافة مصفوفة كاملة إلى المصفوفة الحالية.
* إضافة دالة `هات_اعتماديات_البناء` (`getBuildDependencies`) إلى الوحدة `شـبكة` (`Net`).
* إضافة الأصناف `خـطأ` (`Error`) و `خـطأ_عام` (`GenericError`) و `لـا_مضمون` (`Possible`) و `بـعدم` (`Nullable`).
...
* إصلاح مشكلة في العبارات التمهيدية المتداخلة تتسبب أحيانًا في حشر الشفرة المولدة في المكان الخطأ.
* إصلاح مشكلة انتهاك الوصول (segfault) أثناء ترجمة بعض الحالات من حلقات السندات المغلقة.

### إصلاحات المكتبات المعيارية

* إصلاح مشكلة في تهيئة مؤشرات المغلفات.
* إصلاح علة في الدالات المغلفة تسبب تضاربًا مع أي متغير حمولة بالاسم `d`.
* إسلاح تعريف دالة `مـتم.شـبكة.كـرل_سهل.هات_معلومة` (`Srl.Net.CurlEasy.getInfo`).
* إصلاح مشاكل في تعريب وحدة `مـتم.شـبكة` (`Srl.Net`).
* إصلاح علة في `مثل_سندنا` (`castSrdRef`).

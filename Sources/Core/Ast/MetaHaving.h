/**
 * @file Core/Ast/MetaHaving.h
 * Contains the header of interface Core::Ast::MetaHaving.
 *
...
#ifndef CORE_AST_METAHAVING_H
#define CORE_AST_METAHAVING_H
...
class MetaHaving : public TiInterface
{
  ...
  INTERFACE_INFO(MetaHaving, TiInterface, "Core.Ast", "Core", "alusus.org");
  ...
  public: virtual SharedPtr<SourceLocation> const& findSourceLocation() const
  {
    ...
    if (sl == 0) {
      Containing<Node> const *container = this->getTiObject()->getInterface<Containing<Node> const>();
      if (container != 0) {
...
#define IMPLEMENT_METAHAVING(type) \
  ...
  private: Core::Basic::SharedPtr<Core::Ast::SourceLocation> sourceLocation; \
  ...
  public: virtual void setSourceLocation(Core::Basic::SharedPtr<Core::Ast::SourceLocation> const &loc) \
  { \
    this->sourceLocation = loc; \
  } \
  public: virtual Core::Basic::SharedPtr<Core::Ast::SourceLocation> const& getSourceLocation() const \
  { \

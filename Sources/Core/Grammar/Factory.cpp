/**
 * @file Core/Grammar/Factory.cpp
 * Contains the implementation of class Core::Grammar::Factory.
 *
...
void Factory::setRoot(Module *grammarRoot)
{
...
void Factory::removeProdsFromGroup(Char const *qualifier, std::vector<Char const*> prods)
{
  ...
  for (auto prod: prods) {
    auto ref1 = Core::Grammar::createReference(prod, &this->referenceCache);

...
### الوسيط: احـتواء[صـنف_المحتوى: صنف] (Containing[ContentType: type])
...
```
  عملية هذا.حدد_عنصرا (تسلسل_العنصر: صـحيح، قيمة: سند[صـنف_المحتوى])؛
```
...
```
  عملية هذا.هات_عنصرا (تسلسل_ألعنصر: صـحيح): سند[صـنف_المتحوى]؛
```
...
### الوسيط: احـتواء_مرن[صـنف_المحتوى: صنف] (DynamicContaining[ContentType: type])
...
```
  عملية هذا.أضف_عنصرا (القيمة: سند[صـنف_المحتوى]): صـحيح؛
```
...
```
  عملية هذا.احشر_عنصرا (تسلسل: صـحيح، قيمة: سند[صـنف_المحتوى])؛
```
...
### الوسيط: احـتواء_تطبيقي[صـنف_المحتوى: صنف] (MapContaining[ContentType: type])
...
```
  عملية هذا.حدد_عنصرا (
    اسم_ألعنصر: مؤشر[مصفوفة[مـحرف]]، قيمة: سند[صـنف_المحتوى]
  ): صـحيح؛
```
...
```
  عملية هذا.هات_عنصرا (اسم_العنصر: مؤشر[مصفوفة[مـحرف]]): سند[صـنف_المحتوى]؛
```
...
### الوسيط: احـتواء_تطبيقي_مرن[صـنف_المحتوى: صنف] (DynamicMapContaining[ContentType: type])
...
```
  عملية هذا.أضف_عنصرا (
    اسم_العنصر: مؤشر[مصفوفة[مـحرف]]، قيمة: سند[صـنف_المحتوى]
  ): صـحيح؛
```
...
```
  عملية هذا.احشر_عنصرا (
    تسلسل: صـحيح، اسم: مؤشر[مصفوفة[مـحرف]]، قيمة: سند[صـنف_المحتوى]
  )؛
```
...
معرف داخل الوحدة `الـقلب.شـبم (Core.Ast)`.

مُشتق من `كـائن_بهوية`.
جذر أصناف شجرة البنية المجردة. يمكن هذا الصنف من الوصول من كائن إلى مالكه.
